...
class BasicBlock(tf.keras.layers.Layer):
    ...
    def call(self, inputs, training, use_residual=True):
        self.cb = []
        x = self.conv1(inputs)
        scale_bn1 = 1.0 / tf.math.sqrt(tf.math.reduce_variance(x)+1e-6)
        ...
        # Add intermediate result (without residual connection)
        self.cb.append(x * scale_bn2 * scale_bn1)
